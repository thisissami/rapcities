...
void setUpArtists(){
    artists.clear();
    String jsonstring = "http://localhost:8888/getArtists?all=7";/*?maxX="+maxX+"&minX="+minX+
              "&minY="+minY+"&maxY="+maxY;*/
	...
	/*	    return;
		...
		//getSong(artist.topTracks[curSong].id);
			...
			if(yes){
				...
				data: {link: link, sponsor: sponsor, title: title, x: map(mouseX, 0, width, minX, maxX), y: map(mouseY-28, 0, height, minY, maxY)}, 
				success: function(data){
					if(data){
						alert("Successfully added video: '" + data.link + "'!");
						
					if(data.sponsor == 'culture'){
						culture.add(data);
					}
					else
						sponsors.add(data);
